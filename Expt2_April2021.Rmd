...
biovolumes_calculations_21_6_20 <- read_excel("copy_biovolumes_calculations_21.6.20.xlsx", 
...
## >>Fig 2 all biomass: this one is cool<<
...
ggsave("march2022figs/fig2.jpg", width = 180, height = 180, units = "mm", scale = 1, dpi = 300, device = "jpg")
...
## >>Fig2 option2 mean and CI div fun plot
...
ggsave("march2022figs/fig2option2.jpg", width = 180, height = 130, units = "mm", scale = 1, dpi = 300, device = "jpg")
...
A <- full_df %>% 
  ...
  geom_jitter(width = 0.2, shape = 21, size =3, alpha = .7, aes(fill = as.factor(strains)))+
  geom_smooth(method = "lm", size = 2, alpha = 0.2, aes(color = as.factor(strains)))+
...
B <- full_df %>% 
  ...
  geom_jitter(width = 0.2, shape = 21, size =3, alpha = .7, aes(fill = as.factor(species)))+
  geom_smooth(method = "lm", size = 2, alpha = 0.2, aes(color = as.factor(species)))+
  scale_fill_brewer(palette = "Dark2")+
  scale_color_brewer(palette = "Dark2")+
  scale_x_continuous(breaks=c(1,2,3))+
  theme(legend.position ="top",
...
CD <- plot_grid(C, D, labels = c("C", "D"))
CD

fig3 <- plot_grid(AB, CD, nrow = 2, hjust = TRUE, vjust = TRUE)
fig3

ggsave("march2022figs/fig3.png")
...
# >>>>May 5 for thesis: linear model with all the interactions
```{r}
...
# Environment -> size effects

```{r}

full_df %>% 
  filter(nuts=="high") %>% 
  ggplot(aes(temp, mean_biov))+
  #geom_boxplot()+
  geom_jitter()+
  scale_y_log10()
...
## model with CV size, only using high nuts
...
model_ME_species_comb <- psem(lmer(maxRFU ~ species*strains + CV_biov + (1|div_tx), df_for_sem), 
              lm(CV_biov ~ species*strains, df_for_sem))
