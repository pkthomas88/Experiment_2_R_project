...
pacman::p_load(tidyverse, nlme, lme4, lmerTest, car, MASS, broom, broom.mixed, ggridges, readxl, ggpubr, mgcv, piecewiseSEM, RColorBrewer, cowplot, scales, grid, lm.beta)
...
ggsave("July2023figs/fig2.jpg", width = 180, height = 180, units = "mm", scale = 1, dpi = 300, device = "jpg")
...
ggsave("July2023figs/fig3.jpg", width = 180, height = 180, scale = 1, units = "mm", dpi = 600, device = "jpeg")
...
# full_df2 <- full_df %>% mutate(nuts = as.numeric(nuts), temp = as.numeric(temp))
# 
# full_df2 %>% ungroup() %>% distinct(temp, nuts)
...
modfullcoefs <- tidy(lmbeta.mod_full, conf.int = TRUE) %>% 
  mutate(sign = ifelse(p.value<0.05, "*", "n.s."))
write_csv(modfullcoefs, "July2023figs/modfullcoefs.csv")
...
mod_GR <- tidy(mod_full_gr, conf.int = TRUE) %>% 
  mutate(sign = ifelse(p.value<0.05, "*", "n.s."))
write_csv(mod_GR, "July2023figs/mod_gr.csv")
...
write_csv(mod5coefs, "July2023figs/mod5coefs.csv")
...
write_csv(mod6coefs, "July2023figs/mod6coefs.csv")
...
write_csv(mod7coefs, "July2023figs/mod7coefs.csv")
...
```{r}
#### Fig 3A - Partial effects of species richness on biomass
...
A <- resid_sp.rfu %>% 
  ...
  geom_smooth(method = 'lm', size = 2, se = FALSE, aes(color = as.factor(strains)))+
...
resid_sp.sizevar <- bind_cols(resid_sp.sizevar, sp_str)
...
AB <- ggarrange(A, B, labels = c("A", "B"), common.legend = TRUE, ncol = 2, nrow = 1, align = "hv", label.y = 1.05)
...
C <- resid_str.rfu %>% 
  ...
  geom_smooth(method = 'lm', size = 2, se = FALSE, aes(color = as.factor(species)))+
  #geom_smooth(method = 'lm', se = FALSE, color = "black")+
    guides(color = "none",
         fill = guide_legend(override.aes = list(size=4)))+
  scale_color_brewer(palette = "Dark2")+
...
E <- resid_sizevar.biomass %>% 
  ggplot(aes(xresid, yresid))+
  geom_point(shape = 21, size = 2, alpha = 0.4, fill = jevergreen)+
...
ggsave("July2023figs/fig3.jpg", width = 150, height = 180, scale = 1, units = "mm", dpi = 600, device = "jpeg")
