...
pacman::p_load(tidyverse, nlme, lme4, lmerTest, car, MASS, broom, broom.mixed, ggridges, readxl, ggpubr, mgcv, piecewiseSEM, RColorBrewer, cowplot)
...
#ggsave("march2023figs/fig2.jpg", width = 180, height = 180, units = "mm", scale = 1, dpi = 300, device = "jpg")
...
ggsave("march2023figs/FigSX_growth_rates.jpg", width = 180, height = 130, units = "mm", scale = 1, dpi = 300, device = "jpg")
...
# Fig 3 (8 panel version)
# ---- need to think about how best to show all this data. Does it make sense to make this an 8-panel figure with sp and strains on the x and biomass, growth rate, mean size, and var size as y facets?? I think maybe

# -->also, does it make more sense to show partial correltions as in Grace et al Nature 2016?
...
## GROWTH RATE ## leave this out for now- it doesn't really fit, 8 panels is too much and gr is not even linked to a hypothesis
# C <- full_df %>% 
#   filter(nuts=="high") %>% 
#   ggplot(aes(species, mumax_exp, 
#              #fill = as.factor(strains), color = as.factor(strains)
#              ))+
#   geom_jitter(width = 0.1, shape = 21, size = 1, alpha = .7, aes(fill = as.factor(strains)))+
#   geom_smooth(method = "lm", size = 1, alpha = 0.2, aes(color = as.factor(strains)))+
...
E <- full_df %>% 
  ...
  theme(legend.position ="none")+
  labs(x = "Species richness",
       y = bquote("Mean cell volume" ~(µm^3)),
       fill = "Strain richness")+
  scale_y_log10()
E
...
EF <- plot_grid(E, F, labels = c("C", "D"), label_size = 8)
EF
...
GH <- plot_grid(G, H, labels = c("E", "F"), label_size = 8)
GH

fig3 <- plot_grid(AB, 
                  #CD,
                  EF,
                  GH, nrow = 3, hjust = TRUE, vjust = TRUE)
fig3

ggsave("march2023figs/fig3.jpg", width = 140, height = 173, scale = 1, units = "mm", dpi = 600, device = "jpeg")
...
# Linear model allowing for all interactions
```{r}
...
AIC(model_FE)
plot(model_FE)
coefs <- summary(model_FE)$coefficients
write_csv(coefs, "Table1_growth_and_biomass_SEM.csv")
...
AIC(model_ME)
plot(model_ME)
coefs <- summary(model_ME)$coefficients
```
...
```{r}
# NO LOG TRANSFORM
model_FE_nogrowthrate <- psem(
  lm(maxRFU ~ species*strains + temp, df_for_sem),
  lm(logvar ~ species*strains + temp, df_for_sem),
  lm(mean_biov ~ species*strains + temp, df_for_sem),
  maxRFU %~~% mean_biov,
  maxRFU %~~% logvar,
...
summary(model_FE_nogrowthrate)
AIC(model_FE_nogrowthrate)
```
...
model_FE_nogrowthrate_log <- psem(
  lm(logmaxRFU ~ species*strains + temp, df_for_sem),
  lm(logvar ~ species*strains + temp, df_for_sem),
  lm(logmeanbiov ~ species*strains + temp, df_for_sem),
  logmaxRFU %~~% logmeanbiov,
...
summary(model_FE_nogrowthrate_log)
AIC(model_FE_nogrowthrate_log)

# LOG TRANSFORMING EVERYTHING
model_FE_growthrate_log <- psem(
  lm(logmaxRFU ~ species*strains + temp, df_for_sem),
  lm(logvar ~ species*strains + temp, df_for_sem),
  lm(logmeanbiov ~ species*strains + temp, df_for_sem),
  logmaxRFU %~~% logmeanbiov,
...
summary(model_FE_growthrate_log)
AIC(model_FE_growthrate_log)
```


```{r}
# LOG TRANSFORMING EVERYTHING AND DELETING MEAN
model_FE_nogrowthrate_log <- psem(
  lm(logmaxRFU ~ species*strains + temp, df_for_sem),
  lm(logvar ~ species*strains + temp, df_for_sem),
  #lm(logmeanbiov ~ species*strains + temp, df_for_sem),
...
summary(model_ME_nogrowthrate)
AIC(model_ME_nogrowthrate)
...
full_df_fits <- full_df %>% 
  group_by(nuts_temp) %>% 
  do(model = tidy(lm(maxRFU ~ species*strains, data = .), conf.int = TRUE)) %>% 
  unnest(model) %>% 
  mutate(sig = ifelse(p.value<0.05, "*", "NA"))

write_csv(full_df_fits, "biomass_by_envt.csv")
